...
public class RotateMainCamera : MonoBehaviour
{
	...
	void Start()
	{
		...
		Button up = rotateUp.GetComponent<Button>();
		up.onClick.AddListener(rotateUpClick);

		Button down = rotateDown.GetComponent<Button>();
		down.onClick.AddListener(rotateDownClick);
	}
